...
docker run -t -d --name cp -v $(pwd)/config:/opt/ngic/config woojoong/kone-cp:lite bash
docker network connect brs11 cp
docker network connect brspgw cp

docker run -t -d --name dp --cap-add IPC_LOCK --cap-add NET_ADMIN --ulimit=memlock=-1 -v $(pwd)/config:/opt/ngic/config woojoong/kone-dp:lite bash
